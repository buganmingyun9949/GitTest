    ...
    public class ExpireCardVM : MainViewModel, IView
    {
                           ...
                           (action) =>
                           {
                               if (!string.IsNullOrEmpty(TxtCardNo))
                               {
                                   ...
                                   Application.Current.Dispatcher.Invoke(new Action(() =>
                                   {
                                       ...
                                       //开始绑定
                                       var result = WebApiProxy.GetHtmlRespInfo(model, ApiType.UserBindCard, GlobalUser.USER.AccessToken);
                                       if (result.retCode == 1)
                                       {
                                           ErrMsgOut = "";
                                           //绑定成功,卡片内容
        ...
        private async void ExecuteRunExtendedDialog(object o)
        {
            ...
            var view = new BindCardOKDialog()
            {
                ...
                {
                    CardNo = GlobalUser.USER.Card.card_id,
                    CardName = GlobalUser.USER.Card.CardName,
                    Validity = $"{GlobalUser.USER.Card.used_time} - {GlobalUser.USER.Card.expire_time}"
                }
            };
