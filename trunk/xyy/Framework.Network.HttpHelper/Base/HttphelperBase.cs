...
using Framework.Network.HttpHelper.Static;

namespace Framework.Network.HttpHelper.Base
{
    /// <summary>
    ...
    internal class HttphelperBase
    {
        ...
        private void GetData(HttpItem item, HttpResult result)
        {
            ...
            {
                //GZIIP处理
                if (item.IsGzip || response.ContentEncoding.Equals("gzip", StringComparison.InvariantCultureIgnoreCase))
                {
                    using (MemoryStream _stream = new MemoryStream())
                    {
                        ...
                        //处理网页Byte
                        byte[] enByte =  _stream.ToArray();

                        if (enByte != null && enByte.Length > 0)
                        {
                            //得到返回的HTML
