...
namespace ST.Common.ToolsHelper
{
    ...
    public class MachineInfoHelper
    {
        ...
        public static Log_Device LD { get; set; }

        public Log_Device GetDevice()
        {
            if (LD != null) return LD;

            LD = new Log_Device();
            LD.platform = "Windows";
            LD.systemVersion = $"{GetSystemName()}_{Environment.OSVersion.VersionString}";
            LD.currentVersion = LD.version = GlobalUser.VER;
            LD.maxMemory = $"{GetPhysicalMemory()}G,{GetAvailablePhysicalMemory()}G";
            LD.buildNumber = $"{GetCPUName()}";//_{GetCPUVersion()}";
            LD.brand =  $"{GetBoardManufacturer()}_{GetBoardID()}_{GetBoardType()}";
            LD.uniqueId = $"{GetCPUID()}_{GetDiskSerialNumber()}";
            LD.width = SystemParameters.PrimaryScreenWidth.ToString();
            LD.height = SystemParameters.PrimaryScreenHeight.ToString();
        ...
        public string GetPhysicalMemory()
        {
            try
            {
                ...
                foreach (ManagementObject mo in moc)
                {
                    ...
                    st += ((Math.Round(
                        Int64.Parse(mo.Properties["TotalPhysicalMemory"].Value.ToString()) / 1024 / 1024 / 1024.0, 1)));
                }
                moc.Dispose();
                mc.Dispose();
                return st;
        ...
        public string GetAvailablePhysicalMemory()
        {
            try
            {
                ...
                {
                    st += ((Math.Round(Int64.Parse(mo2.Properties["AvailableMBytes"].Value.ToString()) / 1024.0, 1)));
