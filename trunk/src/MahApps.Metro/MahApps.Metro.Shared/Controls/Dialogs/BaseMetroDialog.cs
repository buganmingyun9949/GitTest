...
namespace MahApps.Metro.Controls.Dialogs
{
    ...
    public abstract class BaseMetroDialog : ContentControl
    {
        ...
        /// <param name="owningWindow">The window that is the parent of the dialog.</param>
        /// <param name="settings">The settings for the message dialog.</param>
        protected BaseMetroDialog(MetroWindow owningWindow, MetroDialogSettings settings, string baseMetroDialogName = null)
        {
            this.Initialize(owningWindow, settings, baseMetroDialogName);
        }
        ...
        private void Initialize([CanBeNull] MetroWindow owningWindow, [CanBeNull] MetroDialogSettings settings,
            [CanBeNull] string baseMetroDialogName)
        {
            this.OwningWindow = owningWindow;
            this.DialogSettings =
                this.ConfigureSettings(settings ?? (owningWindow?.MetroDialogOptions ?? new MetroDialogSettings()));
            ...
            {
                Source = new Uri(
                    $"pack://application:,,,/MahApps.Metro;component/Themes/Dialogs/{baseMetroDialogName ?? "BaseMetroDialog"}.xaml")
            });
            if (this.DialogSettings?.CustomResourceDictionary != null)
            {
                this.Resources.MergedDictionaries.Add(this.DialogSettings.CustomResourceDictionary);
            ...
            this.DialogTitleFontSize =
                (this.TryFindResource("DialogTitleFontSize") as double?).GetValueOrDefault(this.DialogTitleFontSize);
            this.DialogMessageFontSize =
                (this.TryFindResource("DialogMessageFontSize") as double?)
                .GetValueOrDefault(this.DialogMessageFontSize);
